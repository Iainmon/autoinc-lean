...
variable (op : Operator α β Δα Δβ (StateT σ IO))

def execIO : StateT σ IO Output := do
...
def execNTimesIO (n : Nat) : StateT σ IO Output := do
...
section
structure Case (α β Δα Δβ σ : Type) where
  op : Operator α β Δα Δβ (StateT σ IO)
...
variable {σ} [Inhabited σ]
variable (case : Case α β Δα Δβ σ)

def Case.exec : StateT σ IO Output := do
  ...
  Experiment.execNTimesIO case.input dx case.op case.rep
...
def Case.exec' : StateT σ IO Unit := do
  printTable
    headers
    (List.singleton $ dumpRes case (← case.exec))

def Case.execCSV : StateT σ IO Unit := do
  IO.println (toCSVString headers (List.singleton $ dumpRes case (← case.exec)))

def Case.execAllCSV' (cases : List (Case α β Δα Δβ σ)) (caption : Option String := none) (i : Nat) : IO Unit := do
  let action : StateT σ IO Unit := do
    let rows ← cases.mapM (
      fun case => do pure $ dumpRes case (← case.exec)
    )
    let csv := toCSVString headers rows
    IO.FS.writeFile s!"data_{i}.csv" csv
  discard <| action default

def Case.execAll' (cases : List (Case α β Δα Δβ σ)) (caption : Option String := none) : IO Unit := do
  let action : StateT σ IO Unit := do
    let rows ← cases.mapM (
      fun case => do pure $ dumpRes case (← case.exec)
    )
...
def Case.execAllCSV (cases : List (Case α β Δα Δβ σ)) (caption : Option String := none) : StateT σ IO Unit := do
  let rows ← cases.mapM (
    fun case => do pure $ dumpRes case (← case.exec)
  )
  let csv := toCSVString headers rows
